...
<div class="row my-5" >
  ...
  <h2>Quản Lý Đơn Đặt Hàng</h2>
 
  <table id="myTable" class="table table-hover table-striped table-responsive-lg">
    <thead>
        <tr>
    ...
    <tbody>
        @foreach ($list as $row)
            <tr>
                ...
                <td>
                    @if ($row->status==1)
                    <a class="btn btn-light" href=""><i class="fas fa-toggle-on"></i>Đang Chờ</a>
                    @elseif($row->status==2)
                ...
                <td>
               <a href="{{ Route('orderdetail',['code'=>$row->id]) }}" class="btn btn-light" ><i class="fas fa-envelope-open-text"></i></a> </td>
            </tr>
        @endforeach
    </tbody>
</table>
